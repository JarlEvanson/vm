...
use crate::{EL, Granule, PhysicalAddressSpaceSize};
...
impl TcrEL1 {
    ...
    /// Returns the size offset of the memory region addressed by TTBR0_EL1.
    pub const fn size_offset_0(self) -> u8 {
        (self.0 & 0x3F) as u8
    }

    /// Sets the size offset of the memory region addressed by TTBR0_EL1.
    ...
    /// Returns the translation [`Granule`] size for TTBR0_EL1.
    ...
    /// Sets the translation [`Granule`] size for TTBR0_EL1.
    ...
    /// Returns the size offset of the memory region addressed by TTBR1_EL1.
    pub const fn size_offset_1(self) -> u8 {
        ((self.0 >> 16) & 0x3F) as u8
    }

    /// Sets the size offset of the memory region addressed by TTBR1_EL1.
    pub const fn set_size_offset_1(self, size: u8) -> Self {
        Self((self.0 & !(0x3F << 16)) | (size as u64 & 0x3F) << 16)
    }

    /// Returns `true` if a translation table walk is *not* performed on a TLB miss, for an address
    /// that is translated using TTBR1_EL1.
    ...
    /// Returns the translation granule size for TTBR1_EL1.
    ...
    /// Sets the translation [`Granule`] size for TTBR1_EL1.
