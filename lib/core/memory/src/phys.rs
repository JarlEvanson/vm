//! Abstractions over physical memory.

use crate::address::{AddressSpaceDescriptor, PhysicalAddress};

/// A trait representing low-level access to physical memory.
///
/// The [`PhysicalMemorySpace`] trait provides an abstraction for reading from and writing to
...
///
/// This is not intended to access MMIO or other device memory/registers.
#[expect(clippy::missing_safety_doc)]
#[expect(clippy::missing_errors_doc)]
pub unsafe trait PhysicalMemorySpace {
    /// Various errors that can occur while reading or writing [`PhysicalMemorySpace`].
    type Error: core::error::Error;

    /// Returns the [`AddressSpaceDescriptor`] that describes the [`PhysicalMemorySpace`].
    fn descriptor(&self) -> AddressSpaceDescriptor;

    /// Reads a single byte from the [`PhysicalMemorySpace`].
    unsafe fn read_u8(&self, address: PhysicalAddress) -> Result<u8, Self::Error>;
    /// Reads a little-endian u16 from the [`PhysicalMemorySpace`].
    unsafe fn read_u16_le(&self, address: PhysicalAddress) -> Result<u16, Self::Error>;
    /// Reads a bit-endian u16 from the [`PhysicalMemorySpace`].
    unsafe fn read_u16_be(&self, address: PhysicalAddress) -> Result<u16, Self::Error>;
    /// Reads a little-endian u32 from the [`PhysicalMemorySpace`].
    unsafe fn read_u32_le(&self, address: PhysicalAddress) -> Result<u32, Self::Error>;
    /// Reads a big-endian u32 from the [`PhysicalMemorySpace`].
    unsafe fn read_u32_be(&self, address: PhysicalAddress) -> Result<u32, Self::Error>;
    /// Reads a little-endian u64 from the [`PhysicalMemorySpace`].
    unsafe fn read_u64_le(&self, address: PhysicalAddress) -> Result<u64, Self::Error>;
    /// Reads a big-endian u64 from the [`PhysicalMemorySpace`].
    unsafe fn read_u64_be(&self, address: PhysicalAddress) -> Result<u64, Self::Error>;

    /// Writes a single byte into the [`PhysicalMemorySpace`].
    unsafe fn write_u8(&mut self, address: PhysicalAddress, value: u8) -> Result<(), Self::Error>;
    /// Writes a little-endian u16 into the [`PhysicalMemorySpace`].
    unsafe fn write_u16_le(
        &mut self,
        address: PhysicalAddress,
        value: u16,
    ) -> Result<(), Self::Error>;
    /// Writes a big-endian u16 into the [`PhysicalMemorySpace`].
    unsafe fn write_u16_be(
        &mut self,
        address: PhysicalAddress,
        value: u16,
    ) -> Result<(), Self::Error>;
    /// Writes a little-endian u32 into the [`PhysicalMemorySpace`].
    unsafe fn write_u32_le(
        &mut self,
        address: PhysicalAddress,
        value: u32,
    ) -> Result<(), Self::Error>;
    /// Writes a big-endian u32 into the [`PhysicalMemorySpace`].
    unsafe fn write_u32_be(
        &mut self,
        address: PhysicalAddress,
        value: u32,
    ) -> Result<(), Self::Error>;
    /// Writes a little-endian u64 into the [`PhysicalMemorySpace`].
    unsafe fn write_u64_le(
        &mut self,
        address: PhysicalAddress,
        value: u64,
    ) -> Result<(), Self::Error>;
    /// Writes a big-endian u64 into the [`PhysicalMemorySpace`].
