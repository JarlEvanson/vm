    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::MappingMismatch => {
                write!(f, "provided input and output ranges were not the same size")
            }
            Self::InvalidRange => write!(
                f,
                "provided input range or output range was not valid\
                according to their address space descriptor"
            ),
            Self::OverlapError => write!(
                f,
                "the requested input region already contained mappings\
                and MapFlags::MAY_OVERWRITE was not set"
            ),
