...
impl Medium for [u8] {
    ...
    fn read_slice(&self, offset: u64, slice: &mut [u8]) -> Result<(), MediumError<Self::Error>> {
        ...
        #[expect(
            ...
            reason = "check_bounds_usize() prevents truncation"
        )]
        slice.copy_from_slice(&self[offset as usize..][..slice.len()]);
        Ok(())
    }
}
...
impl BackedMedium for [u8] {
    fn access_slice(&self, offset: u64, length: u64) -> Result<&[u8], MediumError<Self::Error>> {
        ...
        #[expect(
            clippy::cast_possible_truncation,
            reason = "check_bounds() prevents truncation"
        )]
        Ok(&self[offset as usize..][..length as usize])
    }
}
