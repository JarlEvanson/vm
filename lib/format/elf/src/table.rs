...
impl<'slice, M: Medium + ?Sized, C: Copy, E: Encoding, T: TableItem<'slice, M, C, E>>
    Table<'slice, M, C, E, T>
{
    ...
    pub fn new(
        ...
        if stride < T::expected_size(class) {
            return None;
        }

        let total_size = count.checked_mul(stride)?;
...
impl<'slice, M: Medium + ?Sized, C: Copy, E: Encoding, T: TableItem<'slice, M, C, E>> IntoIterator
...
impl<'slice, M: Medium + ?Sized, C: Copy, E: Encoding, T: TableItem<'slice, M, C, E> + fmt::Debug>
...
impl<'slice, M: Medium + ?Sized, C: Copy, E: Encoding, T: TableItem<'slice, M, C, E>> Clone
    for Table<'slice, M, C, E, T>
{
    fn clone(&self) -> Self {
        *self
...
impl<'slice, M: Medium + ?Sized, C: Copy, E: Encoding, T: TableItem<'slice, M, C, E>> Copy
    for Table<'slice, M, C, E, T>
{
}
...
impl<'slice, M: Medium + ?Sized, C: Copy, E: Encoding, T: TableItem<'slice, M, C, E>> Iterator
...
pub trait TableItem<'slice, M: Medium + ?Sized, C: Copy, E: Encoding> {
    /// Creates a new [`Self`], panicking when necessary to maintain safety.
    ///
    /// # Panics
    ///
    /// Panics if the [`TableItem`] cannot fit within the [`Medium`] at `offset`.
