...
impl MemoryType {
    ...
    /// Memory that contains regions of the address space containing the ACPI tables (if they aren't
    /// already inside an ACPI reclaimable or ACPI NVS region), the SMBIOS tables, EFI Runtime
    /// Services, and the EFI system table.
