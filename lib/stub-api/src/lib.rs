...
//!
//! This crate serves as the protocol's specification.
//!
//! ## General Notes
//!
...
//!
//! The ABIs the REVM protocol uses and expects the executable to comply with are as follows:
//! - **aarch64**: AAPCS64 without FP/SIMD
//! - **i686**: System V ABI without FP/SIMD
...
pub struct HeaderV0 {
    /// The base version of the REVM protocol that this table or interface implements.
    pub version: u64,
    /// The last base version that was a major version (namely, it broke API/ABI compatibility from
    /// the last version).
...
///
/// All functions must be called on the main CPU.
#[repr(C)]
...
pub struct GenericTableV0 {
    ...
    /// minimum alignment for physical memory allocation and mapping.
    ///
    ...
    /// The ID of the main CPU.
    pub main_cpu: u64,

    /// The number of CPUs on which [`GenericTableV0::run_on_all_processors`] will run the provided
    /// function.
    pub cpu_count: u64,

    /// Various flags concerning the relationship between the bootloader and the executable.
    pub flags: Flags,

    /// A UTF-8 command line terminated by a NUL-character.
    ...
    /// Allocates a physically contiguous region of `count` frames in accordance with the provided
    /// `flags`. The physical address of the start of the region is a multiple of `alignment` and
    /// is written to `physical_address` on success.
    ...
    /// Deallocates the physically contiguous region of `count` frames that starts at
    /// `physical_address`.
    ...
    /// Maps the physically contiguous region starting at `physical_address` and extending for
    /// `count` frames into the executable's address space starting at `virtual_address` and
    /// extending for `count` pages.
    ...
    /// Unmaps the virtual region starting at `virtual_address` and extending for `count` pages.
    pub unmap: unsafe extern "C" fn(virtual_address: usize, count: usize) -> Status,

    /// Signals to the bootloader that the application wishes to take over control of the system.
    /// On success, all processors execute `procedure`, where `arg` is NULL.
    ///
    /// `key` is should be the current memory map's `key` and is checked to ensure that the
    /// application has the correct memory map.
    ///
    /// After this function succeeds, all services/functions provided by this boot protocol are
    /// invalid and must not be called.
    pub takeover:
        unsafe extern "C" fn(key: u64, flags: TakeoverFlags, procedure: Procedure) -> Status,

    /// Executes the provided function on all processors.
    ///
    /// This includes the boot CPU.
    pub run_on_all_processors: unsafe extern "C" fn(procedure: Procedure, arg: *mut ()) -> Status,
}

/// The function prototype required for [`GenericTable::run_on_all_processors`].
...
impl Flags {
    /// If this bit is set, then the bootloader belongs an application that may be transparently
    /// virtualized.
    pub const MAY_VIRTUALIZE: Self = Self(0b1);
...
impl AllocationFlags {
    ...
    /// Only the available range of frames that starts at the provided address may be returned.
    pub const AT: Self = Self(1);
    /// Any available range of frames that is within the region bounded by the provided inclusive
    /// maximum address may be returned.
    pub const INCLUSIVE_MAX: Self = Self(2);
...
impl TakeoverFlags {
    /// Indicates to the firmware that the application has virtualized the system on all
    /// processors. This means that the bootloader should exit and resume normal execution.
    ///
    /// This also means that the `procedure` will not be called by any processor.
    ///
    /// This flag must only be set when [`Flags::MAY_VIRTUALIZE`] is set.
    pub const VIRTUALIZED: Self = Self(1);

    /// Mask over all valid flags.
    pub const VALID: Self = Self::VIRTUALIZED;
...
pub struct MemoryDescriptor {
    /// The physical memory address at the start of the physical memory region described by this
    /// [`MemoryDescriptor`].
    pub start: u64,
    /// The number of bytes for which this region extends.
    pub count: u64,
...
impl MapFlags {
    ...
    /// The mapping function should choose the virtual location of the mapping (it will ignore the
    /// initial value of `virtual_address` and write the chosen address to `virtual_address`).
    pub const CHOOSE_LOCATION: Self = Self(1 << 4);
    ...
    pub const VALID: Self = Self(
        Self::READ.0
            | Self::WRITE.0
            | Self::EXEC.0
            | Self::MAY_OVERWRITE.0
            | Self::CHOOSE_LOCATION.0,
    );
