...
use conversion::{u64_to_usize_strict, usize_to_u16_strict, usize_to_u64};
...
pub extern "C" fn uacpi_kernel_io_map(
    ...
    crate::debug!("uacpi_kernel_io_map({base:#x}, {length:#x}, {out:p})");

    let x = ptr::with_exposed_provenance_mut(u64_to_usize_strict(base));
    unsafe { *out = x }
...
#[unsafe(no_mangle)]
pub extern "C" fn uacpi_kernel_io_unmap(handle: uacpi_handle) {
    crate::debug!("uacpi_kernel_io_unmap({handle:p})");
}
...
pub extern "C" fn uacpi_kernel_io_read8(
    ...
    let val = unsafe { x86_common::io_port::read_u8(usize_to_u16_strict(handle.addr() + offset)) };
    crate::debug!("uacpi_kernel_io_read8({handle:p}, {offset:#x}, {out_value:p}) -> {val:#04x}");
...
pub extern "C" fn uacpi_kernel_io_read16(
    ...
    let val = unsafe { x86_common::io_port::read_u16(usize_to_u16_strict(handle.addr() + offset)) };
    crate::debug!("uacpi_kernel_io_read16({handle:p}, {offset:#x}, {out_value:p}) -> {val:#06x}");
    unsafe { *out_value = val }
    UACPI_STATUS_OK
}
...
pub extern "C" fn uacpi_kernel_io_read32(
    ...
    let val = unsafe { x86_common::io_port::read_u32(usize_to_u16_strict(handle.addr() + offset)) };
    crate::debug!("uacpi_kernel_io_read32({handle:p}, {offset:#x}, {out_value:p}) -> {val:#010x}");
    unsafe { *out_value = val }
    UACPI_STATUS_OK
}
...
pub extern "C" fn uacpi_kernel_io_write8(
    ...
    crate::debug!("uacpi_kernel_io_write8({handle:p}, {offset:#x}, {in_value:#04x})");
    unsafe { x86_common::io_port::write_u8(usize_to_u16_strict(handle.addr() + offset), in_value) }
    UACPI_STATUS_OK
}
...
pub extern "C" fn uacpi_kernel_io_write16(
    ...
    crate::debug!("uacpi_kernel_io_write16({handle:p}, {offset:#x}, {in_value:#06x})");
    unsafe { x86_common::io_port::write_u16(usize_to_u16_strict(handle.addr() + offset), in_value) }
    UACPI_STATUS_OK
}
...
pub extern "C" fn uacpi_kernel_io_write32(
    ...
    crate::debug!("uacpi_kernel_io_write32({handle:p}, {offset:#x}, {in_value:#010x})");
    unsafe { x86_common::io_port::write_u32(usize_to_u16_strict(handle.addr() + offset), in_value) }
    UACPI_STATUS_OK
}
...
pub extern "C" fn uacpi_kernel_map(
    ...
    let end_address = start_address.strict_add(usize_to_u64(size));
    let range = if start_address == end_address {
        FrameRange::empty()
    } else {
        FrameRange::from_inclusive(
            Frame::containing_address(start_address),
            Frame::containing_address(end_address),
        )
    };

    let offset = start_address.value() % usize_to_u64(page_frame_size());
    let Ok(page_range) =
    ...
    let raw_address = page_range
        .start()
        .start_address()
        .strict_add(u64_to_usize_strict(offset))
        .value();
...
pub extern "C" fn uacpi_kernel_unmap(addr: *mut core::ffi::c_void, size: uacpi_size) {
    ...
    let addr_usize = addr.addr() & !(u64_to_usize_strict(generic_table.page_frame_size - 1));
    let result = unsafe {
        (generic_table.unmap)(
            addr_usize,
            size.div_ceil(u64_to_usize_strict(generic_table.page_frame_size)),
        )
    };
...
pub extern "C" fn uacpi_kernel_get_rsdp(out_rsdp_address: *mut uacpi_phys_addr) -> uacpi_status {
    crate::debug!("uacpi_kernel_get_rsdp({out_rsdp_address:p})");
