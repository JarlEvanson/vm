...
static ARCH_FEATURE_SUPPORT: ControlledModificationCell<ArchCapabilities> =
    ControlledModificationCell::new(ArchCapabilities(
        arch::capabilities::ArchCapabilities::initial(),
    ));
...
pub unsafe fn initialize_arch_capability_support() {
    let arch_capability_support = arch::capabilities::ArchCapabilities::new();
...
pub struct ArchCapabilities(arch::capabilities::ArchCapabilities);

impl core::ops::Deref for ArchCapabilities {
    type Target = arch::capabilities::ArchCapabilities;
...
impl fmt::Debug for ArchCapabilities {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        <arch::capabilities::ArchCapabilities as fmt::Debug>::fmt(&self.0, f)
    }
}
