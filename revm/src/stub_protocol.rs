...
use crate::{arch::virtualization, util::image_start};
...
extern "C" fn revm_entry(header_ptr: *mut HeaderV0) -> Status {
    let result = validate_protocol_table(header_ptr);
    ...
    let generic_table = unsafe { generic_table().unwrap().as_ref() };

    crate::debug!("revm virtual start: {:#x}", image_start());
    crate::debug!(
        "revm physical start: {:#x}",
    ...
    let cr0 = x86_common::control_register::Cr0::from_bits(cr0);
    crate::debug!("{cr0:#?}");
    crate::debug!("{cr0:}");

    let cr4: u64;
    unsafe { core::arch::asm!("mov {}, cr4", lateout(reg) cr4) };
    let cr4 = x86_common::control_register::Cr4::from_bits(cr4);
    crate::debug!("{cr4:#?}");
    crate::debug!("{cr4:}");
...
pub fn protocol_table() -> Option<NonNull<Header>> {
    NonNull::new(PROTOCOL_TABLE.load(Ordering::Acquire))
}

/// Returns the REVM protocol generic table.
pub fn generic_table() -> Option<NonNull<GenericTable>> {
    let Some(header_ptr) = protocol_table() else {
        return None;
    };

    let generic_table_offset_ptr = header_ptr
        .as_ptr()
        .wrapping_byte_add(mem::offset_of!(HeaderV0, generic_table_offset))
        .cast::<u64>();

    // SAFETY:
    //
    // The REVM protocol and [`validate_protocol_table()`] ensure that reading from this location
    // is valid and that `generic_table_offset` is initialized.
    let generic_table_offset = unsafe { generic_table_offset_ptr.read() };
    let generic_table_ptr = header_ptr
        .as_ptr()
        .wrapping_byte_add(generic_table_offset as usize)
        .cast::<GenericTable>();
    NonNull::new(generic_table_ptr)
}

fn validate_protocol_table(header_ptr: *mut HeaderV0) -> Status {
