...
pub fn protocol_table() -> Option<&'static Header> {
    // SAFETY:
    //
    // If [`PROTOCOL_TABLE`] contains a non-NULL pointer, it is a valid [`Header`] pointer.
...
pub fn generic_table() -> Option<&'static GenericTable> {
    ...
    // SAFETY:
    //
    // If [`PROTOCOL_TABLE`] contains a non-NULL pointer, it points to a valid [`GenericTable`]
    // pointer.
    unsafe {
        (&raw const *header)
...
pub fn arch_table() -> Option<&'static ArchTable> {
    ...
    // SAFETY:
    //
    // If [`PROTOCOL_TABLE`] contains a non-NULL pointer, it points to a valid [`ArchTable`]
    // pointer.
    unsafe {
        (&raw const *header)
