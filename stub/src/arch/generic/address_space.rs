...
pub trait AddressSpace {
    ...
    /// Returns the maximum supported physical address.
    ///
    /// This is an inclusive range.
    fn max_physical_address(&self) -> u64;

    /// Mps the physical memory region with a base of `physical_address` and a size of `count *
    /// Self::page_size()` into the [`AddressSpace`] at `virtual_address` with the specified
    /// [`ProtectionFlags`].
    ///
    /// # Errors
    ///
    /// - [`MapError::AlignmentError`]: Returned if the `physical_address` or the `virtual_address`
    ///   is not alinged to the [`AddressSpace::page_size()`].
    /// - [`MapError::AllocationError`]: Returned if an error occurs when allocating memory.
    /// - [`MapError::InvalidAddress`]: Returned if `physical_address` or `virtual_address` is not a
    ///   valid address.
    /// - [`MapError::InvalidSize`]: Returned if the requested mapping is too large.
    /// - [`MapError::OverlapError`]: Returned if the requested mapping overlaps with an existing
    ///   mapping.
    /// - [`MapError::WrapAroundError`]: Returned if the region describd by `physical_address` or
    ///   `virtual_address` overflows.
    fn map(
        &mut self,
        virtual_address: u64,
    ...
    /// # Safety
    ///
    /// The unmapped region must been completely mapped and must not be accessed while it is
    /// unmapped.
...
impl ProtectionFlags {
    ...
    /// Mask over all valid bits.
    pub const VALID: Self = Self(Self::READ.0 | Self::WRITE.0 | Self::EXEC.0);
    ...
    /// Sets whether the page should be readable.
    pub const fn set_readable(self, readable: bool) -> Self {
        #[expect(clippy::as_conversions)]
        Self((self.0 & !Self::READ.0) | (readable as u32))
    }
    ...
    /// Sets whether the page should be writable.
    pub const fn set_writable(self, writable: bool) -> Self {
        #[expect(clippy::as_conversions)]
        Self((self.0 & !Self::WRITE.0) | ((writable as u32) << 1))
    }

    /// Returns `true` if the page should be executable.
    pub const fn executable(self) -> bool {
        self.0 & Self::EXEC.0 == Self::EXEC.0
    }

    /// Sets whether the page should be executable.
    pub const fn set_executable(self, executable: bool) -> Self {
        #[expect(clippy::as_conversions)]
        Self((self.0 & !Self::EXEC.0) | ((executable as u32) << 1))
    }
}
...
pub enum MapError {
    ...
    InvalidSize,
    /// The requested mapping overlaps with a previous mapping.
    OverlapError,
...
impl fmt::Display for MapError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::OverlapError => "requested mapping overlaps with a previous mapping".fmt(f),
