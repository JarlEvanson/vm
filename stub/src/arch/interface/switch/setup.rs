...
use crate::{
    arch::{
        interface::memory::paging::{
            ExternalFrame, ExternalFrameRange, ExternalPhysicalAddress, TranslationScheme,
        },
        memory::ArchTranslationScheme,
        switch::{
    ...
    platform::{
        AllocationPolicy, FrameAllocation, MapError, OutOfMemory, PageMapping, Permissions,
        PhysicalAddress, allocate, allocate_frames_aligned, frame_size, main_processor_id,
        map_identity, processor_count, write_bytes_at, write_u8_at,
    },
    util::DropWrapper,
};
...
/// Returns the contents of [`CpuData`] slice and the provided [`TranslationScheme`].
pub fn switch_data() -> SpinlockGuard<'static, Option<SwitchData>> {
    SWITCH_DATA.lock()
}

/// Clears the stored [`CpuData`] slice and provided [`TranslationScheme`].
pub fn clear() {
...
pub fn setup(
    mut scheme: ArchTranslationScheme,
    policy: AllocationPolicy,
) -> Result<(), CpuDataError> {
    let base_storage = base_cpu_storage(&mut scheme);

    let layout = Layout::array::<Spinlock<CpuData>>(u64_to_usize_strict(processor_count()))
        .expect("CpuData array is too large");

    let cpu_data_array = allocate(layout).ok_or_else(|| todo!())?;
    ...
    let cpu_data_slice = unsafe {
        slice::from_raw_parts_mut(
            ...
            u64_to_usize_strict(processor_count()),
        )
    };
    ...
    let switch_data = SwitchData {
        scheme,
        cpu_data: cpu_data_allocation.cast::<Spinlock<CpuData>>(),
    };
...
fn allocate_cpu_data(
    scheme: &mut ArchTranslationScheme,
    mut base_storage: CpuStorage,
    policy: AllocationPolicy,
) -> Result<CpuData, CpuDataError> {
    let (storage_frame_allocation, storage_page_mapping) =
    ...
    let storage_pointer = storage_frame_allocation.range().start_address().value();
    ...
    let mut cpu_data = CpuData {
        ...
        _storage_mapping: storage_page_mapping,
    };
...
fn allocate_stack(
    ...
    let frame_range = ExternalFrameRange::new(
        ExternalFrame::containing_address(
            ExternalPhysicalAddress::new(frame_allocation.range().start().start_address().value()),
            scheme.chunk_size(),
        ),
        stack_pages,
    );

    scheme.map_identity(frame_range, Permissions::ReadWrite)?;
...
fn allocate_storage(
    ...
    storage: &mut CpuStorage,
    policy: AllocationPolicy,
) -> Result<(FrameAllocation, PageMapping), ComponentError> {
    ...
    let page_mapping = map_identity(frame_allocation.range(), Permissions::ReadWrite)?;

    scheme.map_identity(frame_range, Permissions::ReadWrite)?;

    let storage_base = frame_allocation.range().start().start_address().value();
    handle_storage_allocation(storage, storage_base);
    Ok((frame_allocation, page_mapping))
}
...
pub fn allocate_protocol_table(
    ...
    let arch_table_size = arch_table_size(scheme);
    let arch_table_size_u64 = usize_to_u64(arch_table_size);
    let total_size = arch_table_size.strict_add(command_line.len().strict_add(1));
    ...
    let address = frame_allocation.range().start().start_address();
    let command_line_address = address.strict_add(arch_table_size_u64);
    if arch_table_64_bit(scheme) {
        ...
        let protocol_table = RevmProtocolTable64 {
            ...
            generic_table: GenericTable64 {
                ...
                // TODO: Fix flags.
                flags: Flags(0),
                command_line: command_line_address.value(),
                write: layout.write,
        ...
        let protocol_table = RevmProtocolTable32 {
            ...
            generic_table: GenericTable32 {
                ...
                // TODO: Fix flags.
                flags: Flags(0),
                command_line: u32::try_from(command_line_address.value())
                    .expect("failed to convert command line address to u32"),
    ...
    write_bytes_at(command_line_address, command_line.as_bytes());
    write_u8_at(
        command_line_address.strict_add(usize_to_u64(command_line.len())),
        0,
    );
...
pub struct SwitchData {
    /// The [`ArchTranslationScheme`] used for mapping and translation.
    scheme: ArchTranslationScheme,
    /// The allocation that contains the [`CpuData`] array.
    cpu_data: NonNull<Spinlock<CpuData>>,
}

// SAFETY:
//
// [`SwitchData`] is safe to send across threads.
unsafe impl Send for SwitchData {}
// SAFETY:
//
// [`SwitchData`] is safe to access and manipulate from multiple threads.
unsafe impl Sync for SwitchData {}

impl SwitchData {
    /// Returns an immutable reference to the [`ArchTranslationScheme`].
    pub fn scheme(&self) -> &ArchTranslationScheme {
        self.both_ref().0
    }

    /// Returns a mutable reference to the [`ArchTranslationScheme`].
    pub fn scheme_mut(&mut self) -> &mut ArchTranslationScheme {
        self.both_mut().0
    }
    ...
    /// Returns mutable references to the [`ArchTranslationScheme`] and [`CpuData`] array.
    pub fn both_ref(&self) -> (&ArchTranslationScheme, &[Spinlock<CpuData>]) {
    ...
    /// Returns mutable references to the [`TranslationScheme`] and [`CpuData`] array.
    pub fn both_mut(&mut self) -> (&mut ArchTranslationScheme, &mut [Spinlock<CpuData>]) {
...
impl Drop for SwitchData {
    fn drop(&mut self) {
        let base_ptr = self.cpu_data.as_ptr();
        for i in 0..u64_to_usize_strict(processor_count()) {
...
pub struct CpuData {
    ...
    /// Holder for the mapping of the [`CpuStorage`].
    pub _storage_mapping: PageMapping,
}
...
impl CpuData {
    ...
    fn storage_ptr(&self) -> *mut CpuStorage {
        self.storage.range().start().start_address().value() as *mut CpuStorage
    }
...
pub struct CodeLayout {
    ...
    /// The [`PageMapping`] representing the virtual mapping of the physical memory in which the
    /// code resides.
