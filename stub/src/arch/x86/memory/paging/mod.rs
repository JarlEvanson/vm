...
pub struct ArchTranslationScheme(Inner);

impl ArchTranslationScheme {
    /// Constructs a new [`ArchTranslationScheme`] that is compatible with the provided [`Machine`].
    ...
    /// Constructs a new [`ArchTranslationScheme`] by taking over the existing page tables.
    ///
    /// # Safety
    ///
    /// For the lifetime of this object, the newly created [`ArchTranslationScheme`] must have
    ...
    pub unsafe fn active_current() -> Option<Self> {
        match current_paging_mode() {
            ...
            // SAFETY:
            //
            // The invariants of [`ArchTranslationScheme::active_current()`] ensure that the
...
impl TranslationScheme for ArchTranslationScheme {
