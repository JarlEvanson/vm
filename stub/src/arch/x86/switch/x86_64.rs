...
use crate::{
    arch::{
        ...
        x86::switch::{ArchCodeLayout, CallStorage, CpuStorage, ModeStorage, arch_policy},
    },
    platform::{
        Permissions, allocate_frames_aligned, frame_size, map_identity, write_bytes_at,
        write_u64_at,
    },
};
...
pub fn allocate_code(
    ...
    let page_mapping = map_identity(frame_allocation.range(), Permissions::ReadWriteExecute)?;

    let chunk_range = ExternalFrameRange::new(
        ExternalFrame::containing_address(
            ExternalPhysicalAddress::new(frame_allocation.range().start_address().value()),
            scheme.chunk_size(),
        ),
        code_size_u64.div_ceil(scheme.chunk_size()),
    );

    scheme.map_identity(chunk_range, Permissions::ReadWriteExecute)?;
    ...
    write_bytes_at(frame_allocation.range().start_address(), code_bytes);

    // Fill the `storage_pointer` and `mode_storage_pointer` in the assembly code.
    if !write_u64_at(frame_allocation.range().start_address(), storage_pointer) {
        panic!("failed to write storage pointer")
    }

    if !write_u64_at(
        frame_allocation.range().start_address().strict_add(8),
        own_mode_storage_pointer,
    ) {
        panic!("failed to write own storage pointer")
    }

    if !write_u64_at(
        frame_allocation.range().start_address().strict_add(16),
        other_mode_storage_pointer,
    ) {
        panic!("failed to write other storage pointer")
    }

    let offset = usize_to_u64(code_start_ptr.addr())
        .wrapping_sub(frame_allocation.range().start_address().value());

    let enter_mode_ptr = &raw const X86_64_CODE_ENTER_MODE;
    let enter_mode = usize_to_u64(enter_mode_ptr.addr()).wrapping_sub(offset);

    let call_handler_ptr = &raw const X86_64_CODE_CALL_HANDLER;
    let call_handler = usize_to_u64(call_handler_ptr.addr()).wrapping_sub(offset);

    let call_ptr = &raw const X86_64_CODE_CALL;
    ...
    let run_on_all_processors_ptr = &raw const X86_64_CODE_RUN_ON_ALL_PROCESSORS;
...
unsafe extern "C" {
    ...
    static X86_64_CODE_ENTER_MODE: u8;

    static X86_64_CODE_CALL_HANDLER: u8;
    static X86_64_CODE_CALL: u8;
    ...
    static X86_64_CODE_RUN_ON_ALL_PROCESSORS: u8;
...
global_asm! {
    ...
    "mov [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_0_OFFSET}], rdi",
    "cmp r11, 1",
    "je call_internal_continue",

    "mov [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_1_OFFSET}], rsi",
    "cmp r11, 2",
    "je call_internal_continue",

    "mov [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_2_OFFSET}], rdx",
    "cmp r11, 3",
    "je call_internal_continue",

    "mov [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_3_OFFSET}], rcx",
    "cmp r11, 4",
    "je call_internal_continue",

    "mov [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_4_OFFSET}], r8",
    "cmp r11, 5",
    "je call_internal_continue",

    "mov [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_5_OFFSET}], r9",
    "cmp r11, 6",
    "je call_internal_continue",
    ...
    "mov rbx, [rip + storage_pointer]",
    "mov bl, [rip + {STORAGE_CHANGE_EFER}]",
    "cmp bl, 0",
    "je 5f",
    ...
    "mov [rbx + {MODE_STORAGE_EFER}], eax",
    "mov [rbx + {MODE_STORAGE_EFER} + 4], edx",
    ...
    "mov ecx, [esp + {MODE_STORAGE_ENTER_MODE}]",
    "jmp ecx",
    ...
    // `edi`: storage pointer.
    ".global X86_64_CODE_ENTER_MODE",
    "X86_64_CODE_ENTER_MODE:",
    ...
    "mov r10, [r10 + {MODE_STORAGE_CALL_HANDLER}]",
    ...
    ".global X86_64_CODE_CALL_HANDLER",
    "X86_64_CODE_CALL_HANDLER:",

    "mov r10, [rip + storage_pointer]",

    "mov r11w, [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_FUNC_ID_OFFSET}]",
    "cmp r11w, {ENTER_FUNC_ID}",
    "jne 5f",
    ...
    "mov rdi, [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_1_OFFSET}]",

    "mov r10, [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_0_OFFSET}]",

    "jmp r10",

    "5:",
    "cmp r11w, {EXEC_ON_PROCESSOR_FUNC_ID}",
    "jne 5f",

    // When [`ENTER_FUNC_ID`] is called, the first argument is the function to run, the second is
    // the CPU ID and the third argument is the user-defined argument to the function.
    "mov rdi, [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_1_OFFSET}]",
    "mov rsi, [r10 + {CALL_STORAGE_OFFSET} + {CALL_STORAGE_ARG_2_OFFSET}]",
    ...
    ".global X86_64_CODE_RUN_ON_ALL_PROCESSORS",
    "X86_64_CODE_RUN_ON_ALL_PROCESSORS:",

    "mov r10, {RUN_ON_ALL_PROCESSORS_FUNC_ID}",
    "mov r11, 2",
    ...
    STORAGE_GDT = const { offset_of!(CpuStorage, gdt) },
