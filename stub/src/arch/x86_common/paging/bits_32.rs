...
type ReadU32 = fn(address: u64) -> u32;
/// A function that writes `value` to the given `address`.
type WriteU32 = fn(address: u64, value: u32);
...
impl Bits32AddressSpace {
    ...
    pub unsafe fn active_current(read: ReadU32, write: WriteU32) -> Result<Self, Bits32Error> {
        ...
        let physical_address = unsafe { Cr3::get().to_bits() & 0xFFFF_FFFF };
    ...
    fn allocate_zeroed_table(write: WriteU32) -> Result<FrameAllocation, OutOfMemory> {
        ...
        for i in 0..usize_to_u64(4096 / mem::size_of::<u32>()) {
            write(frame_allocation.physical_address() + i * 4, 0);
        }
        Ok(frame_allocation)
    }
    ...
    unsafe fn free_table_recursive(&self, table_physical_address: u64, level: usize) {
        ...
        for i in 0..1024 {
            let entry_address = table_physical_address + i * 4;
            ...
            unsafe { self.free_table_recursive(u64::from(child_table), level - 1) }
        }
...
impl AddressSpace for Bits32AddressSpace {
    ...
    fn map(
        ...
        for index in 0..count {
            ...
            let pml1_table_address = {
                let pml2e_address = self.physical_address + pml2e_index * 4;
                ...
                if !pml2e.present() {
                    ...
                    pml2e = TranslationDescriptor::non_present()
                        .set_present(true)
                        .set_writable(true)
                        .set_table_address(frame_allocation.physical_address() as u32);
    ...
    unsafe fn unmap(&mut self, virtual_address: u64, count: u64) {
        ...
        for index in 0..count {
            ...
            let pml1e_address = u64::from(pml1_table_address) + pml1e_index * 4;
            (self.write)(
                pml1e_address,
