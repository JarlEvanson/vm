...
type ReadU64 = fn(address: PhysicalAddress) -> u64;
/// A function that writes `value` to the given `address`.
type WriteU64 = fn(address: PhysicalAddress, value: u64);
...
impl LongModeAddressSpace {
    ...
    /// Creates a new [`LongModeAddressSpace`] with the current configuration of the CPU.
    ///
    /// # Errors
    ///
    /// - [`LongModeError::OutOfMemory`]: Returned if the allocation of the root frame failed.
    ...
    fn allocate_zeroed_table(write: WriteU64) -> Result<FrameAllocation, OutOfMemory> {
        ...
        for i in 0..usize_to_u64(4096 / mem::size_of::<u64>()) {
            write(
                frame_allocation.range().start().start_address().add(i * 8),
                0,
            );
        }
        Ok(frame_allocation)
    ...
    unsafe fn free_table_recursive(&self, table_physical_address: PhysicalAddress, level: usize) {
        ...
        for i in 0..512u64 {
            let entry_address = table_physical_address.add(i * 8);
    ...
    fn map(
        ...
        for index in 0..count {
            ...
            let pml4_table_address = if self.la57 {
                let pml5e_address = self.physical_address.add(pml5e_index * 8);
            ...
            let pml3_table_address = {
                let pml4e_address = pml4_table_address.add(pml4e_index * 8);
            ...
            let pml2_table_address = {
                let pml3e_address = pml3_table_address.add(pml3e_index * 8);
            ...
            let pml1_table_address = {
                let pml2e_address = pml2_table_address.add(pml2e_index * 8);
            ...
            let pml1e_address = pml1_table_address.add(pml1e_index * 8);
