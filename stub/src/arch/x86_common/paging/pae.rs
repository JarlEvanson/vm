//! Implementation of `PAE` paging implementation.

use core::{error, fmt, mem};

use x86_32::paging::pae::{PdpteDescriptor, TranslationDescriptor};
use x86_common::{
    control::Cr3,
...
/// Implementation of [`AddressSpace`] for `x86_32` with `PAE` paging.
pub struct PaeAddressSpace {
...
impl PaeAddressSpace {
    /// Creates a new [`PaeAddressSpace`] with the provided flags.
    ///
    /// # Errors
    ///
    ...
    pub fn new(nxe: bool, read: ReadU64, write: WriteU64) -> Result<Self, PaeError> {
    ...
    /// Creates a new [`PaeAddressSpace`] in accordance with the the current paging scheme.
    ///
    /// # Errors
    ///
    /// - [`PaeError::OutOfMemory`]: Returned if the allocation of the root frame failed.
    /// - [`PaeError::NotActive`]: Returned if the active mode is not `PAE` paging.
    pub fn new_current(read: ReadU64, write: WriteU64) -> Result<Self, PaeError> {
        ...
        Self::new(nxe, read, write)
    }

    /// Creates a new [`PaeAddressSpace`] by taking over the current page tables referenced by
    /// `CR3`.
    ///
    /// # Errors
    ///
    ...
    /// # Safety
    ///
    /// For the lifetime of this object, the newly created [`PaeAddressSpace`] must have
    /// exclusive control over the memory making up the page tables.
    pub unsafe fn active_current(read: ReadU64, write: WriteU64) -> Result<Self, PaeError> {
        let paging_mode = current_paging_mode();
        if !(paging_mode == PagingMode::Pae
            || paging_mode == PagingMode::Level4
            || paging_mode == PagingMode::Level5)
        ...
        let physical_address = unsafe { Cr3::get().to_bits() & 0xFFFF_FFE0 };
...
impl AddressSpace for PaeAddressSpace {
    ...
    fn map(
        ...
        for index in 0..count {
            ...
            let pdpte_index = (virtual_address >> 30) & 0b11;
            let pml2e_index = (virtual_address >> 21) & 0x1FF;
            let pml1e_index = (virtual_address >> 12) & 0x1FF;

            let pml2_table_address = {
                let pdpte_address = self.physical_address + pdpte_index * 8;
                let pdpte_value = (self.read)(pdpte_address);
                ...
                if !pdpte.present() {
                    ...
                    pdpte = PdpteDescriptor::non_present()
                        .set_present(true)
                        .set_address(frame_allocation.physical_address());
                    (self.write)(pdpte_address, pdpte.to_bits());
                    mem::forget(frame_allocation);
                }
...
/// Various errors that can occur when creating a [`PaeAddressSpace`].
...
impl From<OutOfMemory> for PaeError {
