...
/// The `func_id` of the page fault handler.
const PAGE_FAULT_ID: u16 = MAX_GENERIC_ID + 1;
...
pub fn switch(
    ...
    let (idt_frame_allocation, idtr) = if stub_long_mode {
        x86_64::allocate_idt(&mut address_space, &executable_code_layout)
            .map_err(SwitchError::Idt)?
    } else {
        todo!()
    };
...
pub extern "C" fn handle_call() -> Status {
    ...
    let result = match storage.call.func_id {
        ...
        PAGE_FAULT_ID => {
            crate::error!("Page Fault:");
            crate::error!("Error Code: {:#x}", storage.call.arg_0);
            crate::error!("RIP: {:#x}", storage.call.arg_1);
            crate::error!("CS: {:#x}", storage.call.arg_2);
            crate::error!("RFLAGS: {:#x}", storage.call.arg_3);
            crate::error!("RSP: {:#x}", storage.call.arg_4);
            crate::error!("SS: {:#x}", storage.call.arg_5);

            panic!("Page Faults cannot be handled");
        }
...
pub enum SwitchError {
    ...
    /// An error occurred while preparing the IDT.
    Idt(ComponentError),
}
...
impl fmt::Display for SwitchError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Idt(error) => write!(f, "error preparing IDT: {error}"),
        }
    }
}
