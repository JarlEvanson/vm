...
pub fn load() -> Result<(AddressSpaceImpl, u64, u64, u64), LoadExecutableError> {
    ...
    let (slide, byte_span) = match elf.header().elf_type()? {
        ElfType::EXECUTABLE => {
        ...
        ElfType::SHARED => {
            ...
            (base - base % alignment, byte_span)
        }
        elf_type => return Err(elf_type.into()),
    };

    debug!("Slide: {slide:#x}");
    debug!("Byte Span: {byte_span:#x}");

    let frame_count = byte_span.div_ceil(address_space.page_size());
        ...
        match header.segment_type()? {
            SegmentType::LOAD => {
                ...
                let segment_physical_address =
                    frames.physical_address() + frame_index * address_space.page_size();
                ...
                write_bytes_at(segment_physical_address + offset, file_bytes);

                let zero_base = segment_physical_address + offset + file_bytes.len() as u64;
    ...
    let entry_point = slide + elf.header().entry()?;
    let virtual_address = slide;
    let physical_address = frames.physical_address();
