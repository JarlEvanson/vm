...
use crate::{
    arch::{
        interface::memory::paging::{ExternalVirtualAddress, TranslationScheme},
...
/// descriptions.
pub fn apply_relocations<T: TranslationScheme>(
    elf: &ParsedElf,
    layout: &Layout,
    scheme: &mut T,
    ...
    for header in elf.program_headers {
        ...
        for dynamic in dynamic_table {
            if dynamic.tag()? == DynamicTag::REL_TABLE {
                trace!("found rel table offset: {:#x}", dynamic.val_ptr()?);
                ...
                trace!("found rela table offset: {:#x}", dynamic.val_ptr()?);
        ...
        'rel_table: {
            ...
            for index in 0..num_entries {
                ...
                let addend = match read_size(elf.machine, relocation_type)? {
                    AddendSize::Bits16 => {
                        read_bytes_from(scheme, relocation_virtual_address, &mut buffer[..2])
                    ...
                    AddendSize::Bits32 => {
                        read_bytes_from(scheme, relocation_virtual_address, &mut buffer[..4])
                            .ok_or(ApplyRelocationsError::OutOfBoundsRelocation)?;
                    ...
                    AddendSize::Bits64 => {
                        read_bytes_from(scheme, relocation_virtual_address, &mut buffer[..8])
                            .ok_or(ApplyRelocationsError::OutOfBoundsRelocation)?;
                ...
                write_bytes_into(scheme, relocation_virtual_address, &buffer[..byte_count])
                    .ok_or(ApplyRelocationsError::OutOfBoundsRelocation)?;
            }
        }
...
/// Reads from the provided [`TranslationScheme`] at `virtual_address` into `bytes`.
#[must_use]
fn read_bytes_from<T: TranslationScheme>(
    scheme: &T,
    virtual_address: u64,
    bytes: &mut [u8],
) -> Option<()> {
    for (index, byte) in bytes.iter_mut().enumerate() {
        ...
        let (_, _, physical_address) =
            scheme.translate(ExternalVirtualAddress::new(virtual_address))?;
        *byte = read_u8_at(PhysicalAddress::new(physical_address.value())).unwrap()
    }

    Some(())
}

/// Writes the provided `bytes` into the provided [`TranslationScheme`] at `virtual_address`.
#[must_use]
fn write_bytes_into<T: TranslationScheme>(
    scheme: &mut T,
    virtual_address: u64,
    bytes: &[u8],
) -> Option<()> {
    for (index, byte) in bytes.iter().enumerate() {
        ...
        let (_, _, physical_address) =
            scheme.translate(ExternalVirtualAddress::new(virtual_address))?;
        if !write_u8_at(PhysicalAddress::new(physical_address.value()), *byte) {
            panic!("failed to write bytes into revm address space")
        }
    }
