...
use memory::address::{Frame, FrameRange, PhysicalAddress, VirtualAddress};
...
pub fn allocate(size: usize, alignment: usize) -> Option<NonNull<u8>> {
    ...
    map(
        frames.start().start_address(frame_size()),
        usize_to_u64(size),
    )
}
...
pub unsafe fn deallocate(ptr: NonNull<u8>, size: usize, _: usize) {
    ...
    unsafe {
        crate::platform::generic::platform().deallocate_frames(FrameRange::new(
            Frame::containing_address(physical_address, frame_size()),
            count,
        ))
    }
}
