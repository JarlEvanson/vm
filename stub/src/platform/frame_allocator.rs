...
use memory::{
    address::{Frame, FrameRange, PhysicalAddress},
...
use crate::platform::{
    AllocationPolicy, BufferTooSmall, MemoryMap, OutOfMemory, StubPhysicalMemory, frame_size,
};
...
static FRAME_ALLOCATOR: Spinlock<FrameAllocator<StubPhysicalMemory>> =
    Spinlock::new(FrameAllocator::new(StubPhysicalMemory));
    ...
    'link_loop: loop {
        ...
        while link != END_LINK {
            let descriptor = MemoryDescriptor {
                number: Frame::containing_address(link, frame_size()).number(),
                count: 1,
            ...
            let Err(()) = frame_allocator.try_insert_region(descriptor) else {
                let link_value =
                    // SAFETY:
                    //
                    // The memory in [`FrameAllocator`] is managed exclusively by the
                    // [`FrameAllocator`].
                    unsafe { StubPhysicalMemory.read_u64_le(link).expect("unreachable") };
    ...
    let result = match policy {
        ...
        AllocationPolicy::At(address) => loop {
            ...
            let range = FrameRange::new(
                Frame::containing_address(PhysicalAddress::new(address), frame_size()),
                count,
            );
        ...
        AllocationPolicy::Below(address) => loop {
            let descriptor = base_iter!()
                .filter(|&descriptor| {
                    ...
                    descriptor_range
                        .end_exclusive()
                        .end_address_exclusive(frame_size())
                        .value()
                        <= address
                })
                .find(|descriptor| descriptor.count >= count)
...
struct FrameAllocator<M: PhysicalMemorySpace> {
    ...
    /// The [`PhysicalMemorySpace`] in which this [`FrameAllocator`] operates.
    memory_space: M,
}

impl<M: PhysicalMemorySpace> FrameAllocator<M> {
    /// Creates an empty [`FrameAllocator`].
    pub const fn new(memory_space: M) -> Self {
        Self {
            link: END_LINK,
            range_count: 0,
            link_count: 0,
    ...
    fn try_insert_region(&mut self, mut descriptor: MemoryDescriptor) -> Result<(), ()> {
        ...
        'link_loop: while current_link != END_LINK {
            ...
            // SAFETY:
            //
            // The active mmeory regions are managed exclusively by [`FrameAllocator`].
            let link_value = unsafe {
                self.memory_space
                    .read_u64_le(current_link)
                    .expect("unreachable")
            };
        ...
        if let Some((link, index)) = upper_overlap_location {
            ...
            upper_descriptor.number = range.end_exclusive().number();
            upper_descriptor.count =
                upper_range.end_exclusive().number() - range.end_exclusive().number();
    ...
    fn allocate_link(&mut self) {
        ...
        while link != END_LINK && current_index <= self.range_count {
            ...
            self.add_link(range.start().start_address(frame_size()));
    ...
    fn add_link(&mut self, link: PhysicalAddress) {
        ...
        if previous_link == END_LINK {
            ...
            unsafe {
                self.memory_space
                    .write_u64_le(self.link, END_LINK.value())
                    .expect("unreachable")
            };
        } else {
            // SAFETY:
            ...
            unsafe {
                self.memory_space
                    .write_u64_le(previous_link, link.value())
                    .expect("unreachable")
            };
            // SAFETY:
            //
            ...
            unsafe {
                self.memory_space
                    .write_u64_le(link, END_LINK.value())
                    .expect("unreachable")
            };
        }
        ...
        for descriptor in iter.clone() {
            ...
            'frame_finder: for frame in descriptor_range.iter() {
                ...
                for link in self.links() {
                    if Frame::containing_address(link, frame_size()) == frame {
                        continue 'frame_finder;
                    }
                }

                return Some(frame.start_address(frame_size()));
            }
        }
    ...
    pub fn links(&self) -> LinkIter<'_, M> {
        LinkIter {
            allocator: self,
    ...
    /// linked list.
    pub fn descriptors(&self) -> DescriptorIter<'_, M> {
        DescriptorIter {
            allocator: self,
            link: self.link,
            index: 0,
    ...
    pub fn next_location(&self, link: &mut PhysicalAddress, index: &mut u64) {
        ...
        if *index >= self.descriptors_per_link {
            ...
            //
            // The active mmeory regions are managed exclusively by [`FrameAllocator`].
            let link_value = unsafe { self.memory_space.read_u64_le(*link).expect("unreachable") };
            *link = PhysicalAddress::new(link_value);
            *index = 0;
            return;
        }
    ...
    pub fn validate_order(&self) {
        ...
        for descriptor in self.descriptors().map(frame_range_from_descriptor) {
            ...
            current_end = descriptor.end_exclusive();
        }
    ...
    pub fn validate(&self) {
        ...
        for link in self.links() {
            let link_frame = Frame::containing_address(link, frame_size());
...
struct LinkIter<'allocator, M: PhysicalMemorySpace> {
    /// The [`FrameAllocator`], which is held to ensure proper lifetimes.
    allocator: &'allocator FrameAllocator<M>,
    /// The current link.
    current_link: PhysicalAddress,
}

impl<M: PhysicalMemorySpace> Iterator for LinkIter<'_, M> {
...
struct DescriptorIter<'allocator, M: PhysicalMemorySpace> {
    /// The [`FrameAllocator`].
    allocator: &'allocator FrameAllocator<M>,
...
impl<M: PhysicalMemorySpace> Iterator for DescriptorIter<'_, M> {
...
fn read_descriptor_at(physical_address: PhysicalAddress) -> MemoryDescriptor {
    ...
    // SAFETY:
    //
    // The memory in [`FrameAllocator`] is managed exclusively by the [`FrameAllocator`].
    #[expect(clippy::multiple_unsafe_ops_per_block)]
    let (number, count, region_type) = unsafe {
        let number = StubPhysicalMemory
            .read_u64_le(physical_address.strict_add(number_offset))
            .expect("unreachable");

        let count = StubPhysicalMemory
            .read_u64_le(physical_address.strict_add(count_offset))
            .expect("unreachable");

        let region_type = StubPhysicalMemory
            .read_u32_le(physical_address.strict_add(region_type_offset))
            .expect("unreachable");

        (number, count, MemoryType(region_type))
    };
...
fn write_descriptor_at(physical_address: PhysicalAddress, descriptor: MemoryDescriptor) {
    ...
    unsafe {
        StubPhysicalMemory
            .write_u64_le(
                physical_address.strict_add(number_offset),
                descriptor.number,
            )
            .expect("unreachable");

        StubPhysicalMemory
            .write_u64_le(physical_address.strict_add(count_offset), descriptor.count)
            .expect("unreachable");
