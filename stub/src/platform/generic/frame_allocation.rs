...
/// # Safety
///
/// The [`AddressChunk`][ac]s contained within the provided [`FrameRange`] must have been
/// allocated by a call to [`allocate_frames()`] or [`allocate_frames_aligned()`] and must not be
/// utilized after this function.
///
/// [ac]: memory::address::AddressChunk
...
pub unsafe fn remove_range(range: FrameRange) {
    ...
    while let Some(mut node) = current {
        ...
        if record.range.end_exclusive() < range.start() {
            previous = current;
            current = record.next;
            continue;
        }
        ...
        // region's end.
        if record.range.start() >= range.end_exclusive() {
            break;
        }
        ...
        let (lower, overlaps, upper) = range.partition(record.range);
        if overlaps != range {
