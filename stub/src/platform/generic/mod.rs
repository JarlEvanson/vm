...
/// Returns the [`Flags`] for which the platform is responsible.
pub fn flags() -> Flags {
    platform().flags()
}
...
/// Returns the total number of active CPUs.
pub fn cpu_count() -> u64 {
    platform().cpu_count()
}

/// Returns the processor ID for the main processor.
pub fn main_processor_id() -> u64 {
    platform().main_processor_id()
}
...
pub fn run_on_all_processors<T>(procedure: Procedure, argument: &T) {
...
///
/// This includes the boot CPU.
pub fn run_on_all_processors_mut<T>(procedure: Procedure, argument: &mut T) {
    run_on_all_processors_raw(procedure, ptr::from_mut(argument).cast::<()>())
}
...
pub fn run_on_all_processors_raw(procedure: Procedure, argument: *mut ()) {
    platform().run_on_all_processors(procedure, argument)
}
...
/// The function prototype required for [`Platform::run_on_all_processors`]
