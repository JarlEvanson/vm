...
pub struct TextConsole<'font, S: Surface> {
    ...
    /// The [`GlyphArray`] containing all glyphs used by the [`TextConsole`].
    glyph_array: GlyphArray<'font>,
    /// The map of [`char`]s to glyphs.
    font_map: FontMap<'font>,
}

impl<'font, S: Surface> TextConsole<'font, S> {
    /// Creates a new [`TextConsole`] that prints characters using the given [`GlyphArray`] and
    ...
    pub fn new(
        ...
        let text_width = surface.width() / usize::from(glyph_array.width());
        let text_height = surface.height() / usize::from(glyph_array.height());
    ...
    pub fn write_char(&mut self, c: char) {
        match c {
            ...
            c => {
                ...
                let Some(glyph) = self.glyph_array.get(u32_to_usize(glyph_index)) else {
                    return;
                };

                let x_base = self.x * usize::from(self.glyph_array.width());
                let y_base = self.y * usize::from(self.glyph_array.height());
    ...
    /// Scrolls the [`TextConsole`] a single line.
    fn scroll(&mut self) {
        ...
        let fill_region = Region {
            ...
            width: self.surface.width(),
            height: usize::from(self.glyph_array.height()),
        };
...
impl<S: Surface> fmt::Write for TextConsole<'_, S> {
