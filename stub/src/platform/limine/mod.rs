...
use limine::{
    ...
    executable_cmdline::{EXECUTABLE_CMD_LINE_REQUEST_MAGIC, ExecutableCmdLineRequest},
...
/// Request for the command line to be passed to the executable.
#[used]
#[unsafe(link_section = ".limine.requests")]
static EXECUTABLE_CMD_LINE_REQUEST: ControlledModificationCell<ExecutableCmdLineRequest> =
    ControlledModificationCell::new(ExecutableCmdLineRequest {
        id: EXECUTABLE_CMD_LINE_REQUEST_MAGIC,
        revision: 0,
        response: ptr::null_mut(),
    });
...
fn validate_required_tables() -> (
    ...
    let cmdline_response_ptr = EXECUTABLE_CMD_LINE_REQUEST.get().response;
    // SAFETY:
    //
    // The Limine bootloader specification states that if the [`EXECUTABLE_CMD_LINE_REQUEST`] pointer
    // changes, the request has been fulfilled. Since `cmdline_response_ptr` was
    // initialized to `ptr::null_mut()`, if it is not `ptr::null_mut()`, it must be a valid
    // pointer.
    let cmdline = match unsafe { cmdline_response_ptr.as_ref() } {
        Some(cmdline_response) => {
            // SAFETY:
            //
            // The Limine bootloader specification states that the
            // [`ExecutableCmdLineResponse::cmd_line`] value is a 0-terminated ASCII string.
            let cstr = unsafe { CStr::from_ptr(cmdline_response.cmd_line) };
            str::from_utf8(cstr.to_bytes()).expect("limine protocol violation")
        }
        None => "",
    };
