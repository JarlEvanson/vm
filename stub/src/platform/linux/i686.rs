...
use crate::{
    ...
    platform::{
        ...
        linux::shared::{E820Iter, acquire_cmdline},
        page_size, register_console, set_rsdp, set_uefi_system_table, set_xsdp,
    },
};
...
pub extern "C" fn linux_main(
    ...
    *PANIC_HANDLER.lock() = panic_handler;
    ...
    // SAFETY:
    //
    // The linux boot protocol ensures that we have exclusive control over the system and that the
    ...
    let command_line = if let Some(cmdline_address) =
        acquire_cmdline(PhysicalAddress::new(boot_params_ptr as u64), false)
    {
