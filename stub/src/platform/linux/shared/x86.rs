...
pub fn acquire_cmdline(boot_params: PhysicalAddress, x86_64: bool) -> Option<PhysicalAddress> {
    assert!(boot_params.is_aligned(usize_to_u64(mem::size_of::<BootParams>())));
    assert!(page_size() >= mem::size_of::<BootParams>());

    let mapping = map_temporary(boot_params)?;
    // SAFETY:
    //
    // The `linux` boot protocol ensures that this operation safe.
    let boot_params = unsafe { &*ptr::with_exposed_provenance::<BootParams>(mapping.value()) };

    let mut cmdline_address = u64::from(boot_params.hdr.cmd_line_ptr);
    if x86_64 {
        cmdline_address |= u64::from(boot_params.ext_cmd_line_ptr) << 32;
    }
    Some(PhysicalAddress::new(cmdline_address))
}
