...
use conversion::{u16_to_usize, u64_to_usize_strict, usize_to_u64};
...
use crate::{
    ...
    platform::{
        ...
        linux::{
            shared::{E820Iter, acquire_cmdline},
            x86_64::virt::setup_initial_mappings,
        },
        map, read_u8_at, register_console,
    },
};
...
pub extern "C" fn linux_main(
    ...
    *PANIC_HANDLER.lock() = panic_handler;
    ...
    let initial_mappings = [(image_start, image_size), (stack_start, stack_size)];
    setup_initial_mappings(&initial_mappings);
    ...
    let (command_line, command_line_memory_region) = if let Some(cmdline_address) =
        acquire_cmdline(PhysicalAddress::new(boot_params as u64), true)
    {
        let mut tracker = cmdline_address;
        while read_u8_at(tracker) != Some(0) {
            tracker = PhysicalAddress::new(tracker.value() + 1);
        }
        // Increment for the final NUL character.
        tracker = PhysicalAddress::new(tracker.value() + 1);
        ...
        let initial_mappings = [
            ...
            (
                cmdline_address.value(),
                (tracker.value() - cmdline_address.value()),
            ),
        ];
        setup_initial_mappings(&initial_mappings);

        let cmdline_ptr = ptr::without_provenance::<core::ffi::c_char>(u64_to_usize_strict(
            cmdline_address.value(),
        ));

        // SAFETY:
        //
        // The linux boot protocol enusres that we have exclusive control over the system and that
        // the provided boot params pointer, as well as the contained command line pointer, is
        // properly aligned and initialized.
        let c_str = unsafe { core::ffi::CStr::from_ptr(cmdline_ptr) };
        match c_str.to_str() {
            Ok(str) => {
                if !str.is_empty() {
                    let memory_region = MemoryDescriptor {
                        range: PhysicalAddressRange::new(cmdline_address, usize_to_u64(str.len())),
                        region_type: MemoryType::BootloaderReclaimable,
                    };

                    (str, Some(memory_region))
                } else {
                    ("", None)
                }
            }
            Err(error) => {
                crate::warn!("failed to acquire command line: {error}");
                ("", None)
            }
        }
    } else {
    ...
    crate::platform::frame_allocator::initialize(
        e820_iter
            ...
            .chain(command_line_memory_region),
    );
    ...
    match crate::stub_main(command_line) {
        Ok(()) => {}
