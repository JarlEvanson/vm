...
pub fn setup_initial_mappings(initial_mappings: &[(u64, u64)]) {
    ...
    let ranges = initial_mappings
        .iter()
        .copied()
        .chain(iter::once((usize_to_u64(TEMP_PAGE_ADDRESS), 4096)));
    for (index, (start_address, size)) in ranges.clone().enumerate() {
