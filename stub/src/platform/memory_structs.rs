...
impl Frame {
    ...
    ///
    /// If `alignment` is less than [`frame_size()`][fs], then [`Frame`] remains the same.
    ///
    /// [fs]: crate::platform::frame_size
...
impl Page {
    ...
    /// `alignment`. `alignment` is an alignment in bytes.
    ///
    /// If `alignment` is less than [`page_size()`][ps], then [`Page`] remains the same.
    ///
    /// [ps]: crate::platform::page_size
    #[expect(clippy::missing_panics_doc, reason = "guard against stupidity")]
