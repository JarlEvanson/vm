...
impl Platform for Uefi {
    ...
    unsafe fn deallocate_frames(&self, physical_address: u64, count: u64) {
        ...
        while remaining != 0 {
            let iter_count = u64_to_usize_panicking(remaining.min(usize_to_u64(usize::MAX)));
    ...
    fn map_temporary(&self, physical_address: u64) -> *mut u8 {
        ...
        ptr::with_exposed_provenance_mut(u64_to_usize_panicking(physical_address))
    }
        ...
        loop {
            ...
            match result {
                ...
                Status::BUFFER_TOO_SMALL => {
                    ...
                    active_ptr = ptr::without_provenance_mut(u64_to_usize_panicking(
                        frame.physical_address(),
                    ));
                    buffer_size = u64_to_usize_panicking(new_frame_count.strict_mul(frame_size()));
