...
use conversion::{
    u64_to_usize_strict, u64_to_usize_truncating, usize_to_u64, usize_to_u64_truncating,
};
use memory::address::{Frame, FrameRange, PhysicalAddress, PhysicalAddressRange, VirtualAddress};
use stub_api::{Flags, TakeoverFlags};
use sync::{ControlledModificationCell, Spinlock};
use uefi::{
    data_type::{Boolean, Event, Guid, Handle, Status, TaskPriorityLevel},
    memory::{MemoryDescriptor, MemoryType},
    protocol::mp::MpServicesProtocol,
    table::{
        boot::{AllocateType, EventType},
...
use crate::platform::{
    AllocationPolicy, BufferTooSmall, OutOfMemory, Platform, allocate, deallocate, deallocate_all,
    deallocate_all_frames, platform_initialize, platform_teardown,
};
...
pub extern "efiapi" fn uefi_main(
    ...
    *crate::PANIC_FUNC.lock() = panic_handler;
    ...
    'mp: {
        use uefi::{protocol::mp::MpServicesProtocol, table::boot::BootServices2_0};
...
impl Platform for Uefi {
    ...
    unsafe fn deallocate_frames(&self, mut range: FrameRange) {
        ...
        while !range.is_empty() {
            ...
            let status = unsafe {
                free_pages_ptr(
                    range.start().start_address(self.frame_size()).value(),
                    iter_count,
                )
            };
            if status.error() {
            ...
            range = FrameRange::from_exclusive(new_start, range.end_exclusive());
        }
    }
    ...
    fn map_temporary(&self, address: PhysicalAddress) -> *mut u8 {
        ...
        assert_eq!(
            usize_to_u64_truncating(u64_to_usize_truncating(address.value())),
            address.value(),
            "can't map physical memory beyond virtual memory region on UEFI"
        );

        ptr::with_exposed_provenance_mut(u64_to_usize_strict(address.value()))
    }

    fn map_identity(&self, range: PhysicalAddressRange) -> *mut u8 {
        assert_ne!(range.start().value(), 0, "can't map zero address on UEFI");
        assert_eq!(
            usize_to_u64_truncating(u64_to_usize_truncating(range.start().value())),
            range.start().value(),
            "can't map physical memory beyond virtual memory region on UEFI"
        );

        let end_address = range.end_exclusive().value();
        assert_eq!(
            usize_to_u64_truncating(u64_to_usize_truncating(end_address)),
            end_address,
            "can't map physical memory beyond virtual memory region on UEFI"
        );

        ptr::with_exposed_provenance_mut(u64_to_usize_strict(range.start().value()))
    }

    fn translate_virtual(&self, address: VirtualAddress) -> Option<PhysicalAddress> {
        Some(PhysicalAddress::new(usize_to_u64(address.value())))
    }

    fn flags(&self) -> Flags {
        Flags::MAY_VIRTUALIZE
    }
    ...
    fn cpu_count(&self) -> u64 {
        ...
        let mut enabled_proc_number = 0;
        // SAFETY:
        //
        // The invariants of [`MpServicesProtocol::get_number_of_processors()`] have been
        // fulfilled.
        let result = unsafe {
            (mp_services.get_number_of_processors)(
                ptr::from_ref(*mp_services).cast_mut(),
                &mut proc_number,
                &mut enabled_proc_number,
            )
        };
        if result == Status::SUCCESS {
    ...
    fn run_on_all_processors(&self, procedure: super::Procedure, argument: *mut ()) {
        if self.cpu_count() == 1 {
        ...
        unsafe extern "efiapi" fn interpose(arg: *mut ffi::c_void) {
            ...
            let mp_services = MP_SERVICES.get().expect("initialization failure");
            ...
            assert_eq!(result, Status::SUCCESS);
            let proc_number = usize_to_u64(proc_number);

            func(proc_number, arg)
        }
