...
use uefi::{
    ...
    table::{
        boot::{AllocateType, BootServices1_1, EventType},
        config,
        system::SystemTable,
    },
};
...
pub extern "efiapi" fn uefi_main(
    ...
    *PANIC_HANDLER.lock() = panic_handler;
    ...
    let (main_processor_id, processor_count) = 'mp: {
        if (boot_services_revision.major() == 1 && boot_services_revision.minor() < 1)
            || boot_services_revision.major() != 2
        {
            crate::warn!(
        ...
        let boot_services_ptr = boot_services_ptr.cast::<BootServices1_1>();

        // SAFETY:
        //
        // `boot_services_ptr` must point to a valid [`BootServices1_1`] table that will contain
        // a `locate_protocol` function pointer since the revision check was successful.
        let locate_protcool_ptr = unsafe { (*boot_services_ptr).locate_protocol };
        ...
        //
        // [`MpServicesProtocol`] will be active until `exit_boot_services()` is called.
        unsafe { *MP_SERVICES.get_mut() = Some(interface_ref) };
        ...
        let result = unsafe {
            (interface_ref.get_number_of_processors)(
                interface,
                &mut processor_count,
        ...
        if result != Status::SUCCESS {
            crate::warn!(
                "MpServices get_number_of_processors failed with {result}: reverting to single-threaded system"
            );
            break 'mp (0, 1);
        }
        ...
        for processor_id in 0..processor_count {
            ...
            let result = unsafe {
                (interface_ref.get_processor_info)(interface, processor_id, &mut processor_info)
            };
            if result != Status::SUCCESS {
                crate::warn!(
                    "MpServices get_processor_info failed with {result}: reverting to single-threaded system"
                );
                break 'mp (0, 1);
            }
    ...
    match crate::stub_main(cmdline.as_str()) {
        Ok(()) => Status::SUCCESS,
        Err(error) => {
            crate::warn!("{error}");
            Status::LOAD_ERROR
...
/// Implementation utilizing the UEFI firmware for most platform abstractions.
struct UefiImpl {
...
impl ProcessorManager for UefiImpl {
    ...
    fn run_on_all_processors(&self, procedure: Procedure, argument: *mut ()) {
        ...
        let system_table_ptr = (*UEFI_SYSTEM_TABLE.lock())
            .expect("illegal call of `run_on_all_processors()`")
            .0;
