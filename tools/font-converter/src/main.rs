//! Tool for converting various bitmap font formats into the custom [`GlyphArray`] and
//! [`FontMapBuilder`] dump formats.

use std::{
    env::args, error, fmt, fs::File, io::Write, num::TryFromIntError, path::Path,
    process::ExitCode, str::Utf8Error,
};

use font::{font_map::FontMapBuilder, glyph::GlyphArray};

fn main() -> ExitCode {
    let font_path = args().nth(1).expect("expected font path");
    let dir_path = args().nth(2).expect("expected output directory path");
    let dir_path = Path::new(dir_path.as_str());

    let font = std::fs::read(font_path).expect("error reading provided font");

    let glyph_array = File::create(dir_path.join("glyph-array.bin")).unwrap();
    let font_map = File::create(dir_path.join("font-map.bin")).unwrap();

    match convert_psf(font.as_slice(), glyph_array, font_map) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("error while converting PSF font: {error}");
            ExitCode::FAILURE
        }
    }
}

/// Parses `font` as a PSF file and writes its corresponding [`GlyphArray`] to `glyph_writer`
/// and the [`FontMap`][fm] to `font_map_writer`.
///
/// # Errors
///
/// Returns an error if any operation produces an internal error.
///
/// # Panics
///
/// May panic if an error occurs.
///
/// [fm]: font::font_map::FontMap
fn convert_psf<W0: Write, W1: Write>(
    font: &[u8],
    glyph_writer: W0,
    font_map_writer: W1,
) -> Result<(), PsfError> {
    if font.len() < 4 {
        return Err(PsfError::TruncatedHeader {
            actual_size: font.len(),
            expected_size: 4,
        });
    }

    let psf_2;
    let glyph_start;
    let glyph_count;
    let glyph_size;
    let glyph_height;
    let glyph_width;
    let has_unicode_table;

    if font[..2] == [0x36, 0x04] {
        // PSF 1
        let mode = font[2];
        let _glyph_size = font[3];

        psf_2 = false;
        glyph_start = 4;
        glyph_count = 256 * (1 + (mode & 0x01 == 0x01) as u32);
        glyph_size = u32::from(_glyph_size);
        glyph_height = glyph_size;
        glyph_width = 8;
        has_unicode_table = (mode & 0x02 == 0x02) || (mode & 0x4 == 0x4);
    } else if font[..4] == [0x72, 0xb5, 0x4a, 0x86] {
        // PSF 2
        if font.len() < 4 * 8 {
            return Err(PsfError::TruncatedHeader {
                actual_size: font.len(),
                expected_size: 32,
            });
        }

        let version = parse_u32(font, 4).unwrap();
        if version != 0 {
            return Err(PsfError::InvalidVersion(version));
        }

        let header_size = parse_u32(font, 8).unwrap();
        let flags = parse_u32(font, 12).unwrap();
        let length = parse_u32(font, 16).unwrap();
        let _glyph_size = parse_u32(font, 20).unwrap();
        let height = parse_u32(font, 24).unwrap();
        let width = parse_u32(font, 28).unwrap();

        psf_2 = true;
        glyph_start = header_size;
        glyph_count = length;
        glyph_size = _glyph_size;
        glyph_height = height;
        glyph_width = width;
        has_unicode_table = flags & 0x00000001 == 0x00000001;
    } else {
        return Err(PsfError::InvalidMagic(parse_u32(font, 0).unwrap()));
    }

    let total_size = glyph_count.strict_mul(glyph_size);
    let max_offset = glyph_height.strict_add(total_size);
    let max_offset_usize = usize::try_from(max_offset)?;
    if font.len() < max_offset_usize {
        return Err(PsfError::TruncatedData {
            actual_size: font.len(),
            expected_size: max_offset,
        });
    }

    let glyph_start_usize = usize::try_from(glyph_start)?;
    let glyph_slice = &font[glyph_start_usize..max_offset_usize];
    let width = u8::try_from(glyph_width)?;
    let height = u8::try_from(glyph_height)?;
    let glyph_array = GlyphArray::new(glyph_slice, width, height);

    let mut font_map;
    let unicode_table_array_start = max_offset_usize;
    if has_unicode_table {
        let base_iter = if psf_2 {
            Psf1Iter(&[]).chain(Psf2Iter(&font[unicode_table_array_start..]))
        } else {
            Psf1Iter(&font[unicode_table_array_start..]).chain(Psf2Iter(&[]))
        };

        let mut iter = base_iter.clone();
        let mut mapping_count = 0;
        'overall: for _ in 0..glyph_count {
            loop {
                let behavior = iter.next().ok_or(PsfError::UnexpectedUnicodeTableEnd)??;
                match behavior {
                    Behavior::Char(_) => mapping_count += 1,
                    Behavior::BeginSeries => break,
                    Behavior::EndEntry => continue 'overall,
                }
            }

            loop {
                let behavior = iter.next().ok_or(PsfError::UnexpectedUnicodeTableEnd)??;
                match behavior {
                    Behavior::Char(_) => continue,
                    Behavior::BeginSeries => continue,
                    Behavior::EndEntry => continue 'overall,
                }
            }
        }

        if iter.next().is_some() {
            return Err(PsfError::TooManyUnicodeEntries);
        }

        iter = base_iter;
        font_map = FontMapBuilder::new((mapping_count * 3) / 2);
        'overall: for glyph_index in 0..glyph_count {
            loop {
                let behavior = iter.next().ok_or(PsfError::UnexpectedUnicodeTableEnd)??;
                match behavior {
                    Behavior::Char(c) => font_map.insert(c, glyph_index).unwrap(),
                    Behavior::BeginSeries => break,
                    Behavior::EndEntry => continue 'overall,
                }
            }

            loop {
                let behavior = iter.next().ok_or(PsfError::UnexpectedUnicodeTableEnd)??;
                match behavior {
                    Behavior::Char(_) => continue,
                    Behavior::BeginSeries => continue,
                    Behavior::EndEntry => continue 'overall,
                }
            }
        }
    } else {
        let glyph_count_usize = usize::try_from(glyph_count)?;
        font_map = FontMapBuilder::new((glyph_count_usize * 3) / 2);
        for i in 0..glyph_count {
            let c = char::from_u32(i).ok_or(PsfError::UnsupportedChar)?;
            font_map.insert(c, i).unwrap();
        }
    }

    glyph_array.dump(glyph_writer);
    font_map.dump(font_map_writer, true);
    Ok(())
}

/// Extracts a little-endian `u16` from `slice` at `offset`.
const fn parse_u16(slice: &[u8], offset: usize) -> Option<u16> {
    let Some(_) = offset.checked_add(4) else {
        return None;
    };

    let Some(bytes) = slice.split_at(offset).1.first_chunk() else {
        return None;
    };

    Some(u16::from_le_bytes(*bytes))
}

/// Extracts a little-endian `u32` from `slice` at `offset`.
const fn parse_u32(slice: &[u8], offset: usize) -> Option<u32> {
    let Some(_) = offset.checked_add(4) else {
        return None;
    };

    let Some(bytes) = slice.split_at(offset).1.first_chunk() else {
        return None;
    };

    Some(u32::from_le_bytes(*bytes))
}

/// The behavior that should happen when parsing a PSF unicode section.
enum Behavior {
    /// Handle a character.
    Char(char),
    /// Begin a new series of unicode characters corresponding to a symbol.
    BeginSeries,
    /// End a unicode entry.
    EndEntry,
}

/// An iterator over the contents of a PSF 1 unicode section.
#[derive(Clone)]
struct Psf1Iter<'slice>(&'slice [u8]);

impl Iterator for Psf1Iter<'_> {
    type Item = Result<Behavior, PsfError>;

    fn next(&mut self) -> Option<Self::Item> {
        let value = parse_u16(self.0, 0)?;
        match value {
            0xFFFE => {
                self.0 = self.0.get(2..)?;
                Some(Ok(Behavior::BeginSeries))
            }
            0xFFFF => {
                self.0 = self.0.get(2..)?;
                Some(Ok(Behavior::EndEntry))
            }
            _ => {
                let c = match char::from_u32(u32::from(value)) {
                    Some(c) => c,
                    None => return Some(Err(PsfError::UnsupportedChar)),
                };

                self.0 = self.0.get(2..)?;
                Some(Ok(Behavior::Char(c)))
            }
        }
    }
}

/// An iterator over the contents of a PSF 2 unicode section.
#[derive(Clone)]
struct Psf2Iter<'slice>(&'slice [u8]);

impl Iterator for Psf2Iter<'_> {
    type Item = Result<Behavior, PsfError>;

    fn next(&mut self) -> Option<Self::Item> {
        let value = self.0.first().copied()?;
        match value {
            0xFE => {
                self.0 = self.0.get(1..)?;
                Some(Ok(Behavior::BeginSeries))
            }
            0xFF => {
                self.0 = self.0.get(1..)?;
                Some(Ok(Behavior::EndEntry))
            }
            _ => {
                let utf8_len = match value {
                    value if value & 0b1000_0000 == 0 => 1,
                    value if value & 0b1110_0000 == 0b1100_0000 => 2,
                    value if value & 0b1111_0000 == 0b1110_0000 => 3,
                    value if value & 0b1111_1000 == 0b1111_0000 => 4,
                    _ => return Some(Err(PsfError::UnsupportedChar)),
                };
                if self.0.len() < utf8_len {
                    return Some(Err(PsfError::UnexpectedUnicodeTableEnd));
                }

                let c = match str::from_utf8(&self.0[..utf8_len]) {
                    Ok(c) => c,
                    Err(err) => return Some(Err(err.into())),
                };
                let c = c.chars().next()?;

                self.0 = self.0.get(utf8_len..)?;
                Some(Ok(Behavior::Char(c)))
            }
        }
    }
}

/// Various errors that can occur when parsing a PSF file.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum PsfError {
    /// The magic signature is invalid.
    InvalidMagic(u32),
    /// The PSF version is unsupported.
    InvalidVersion(u32),
    /// The header was truncated.
    TruncatedHeader {
        /// The actual size of the header.
        actual_size: usize,
        /// The expected size of the header.
        expected_size: u32,
    },
    /// The PSF data was truncated.
    TruncatedData {
        /// The actual size of the data.
        actual_size: usize,
        /// The expected size of the data.
        expected_size: u32,
    },
    IntegerConversionError(TryFromIntError),
    UnexpectedUnicodeTableEnd,
    TooManyUnicodeEntries,
    UnsupportedChar,
    UnicodeError(Utf8Error),
}

impl fmt::Display for PsfError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidMagic(magic) => write!(f, "invalid magic: {magic:08X}"),
            Self::InvalidVersion(version) => write!(f, "invalid version: {version}"),
            Self::TruncatedHeader {
                actual_size,
                expected_size,
            } => write!(
                f,
                "header is truncated: expected {expected_size} bytes but got {actual_size} bytes"
            ),
            Self::TruncatedData {
                actual_size,
                expected_size,
            } => write!(
                f,
                "data is truncated: expected {expected_size} bytes but got {actual_size} bytes"
            ),
            Self::IntegerConversionError(error) => write!(f, "error converting integer: {error}"),
            Self::UnexpectedUnicodeTableEnd => write!(f, "unexpected unicode table end"),
            Self::TooManyUnicodeEntries => write!(f, "too many unicode table entries"),
            Self::UnsupportedChar => write!(f, "unsupported unicode codepoint"),
            Self::UnicodeError(error) => write!(f, "error decoding unicode: {error}"),
        }
    }
}

impl error::Error for PsfError {}

impl From<TryFromIntError> for PsfError {
    fn from(error: TryFromIntError) -> Self {
        Self::IntegerConversionError(error)
    }
}

impl From<Utf8Error> for PsfError {
    fn from(error: Utf8Error) -> Self {
        Self::UnicodeError(error)
    }
}
